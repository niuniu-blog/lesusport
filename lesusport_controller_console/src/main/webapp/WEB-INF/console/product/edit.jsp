...
<div class="body-box" style="float:right">
    <form id="jvForm" action="/product/updateProduct" method="post" enctype="multipart/form-data">
                    ...
                    <select name="brandId">
                        ...
                        <c:forEach items="${brands}" var="brand">
                            <option value="${brand.id}" <c:if
                                    test="${brand.id == product.brandId}"> selected="selected"</c:if>
                            >${brand.name}</option>
                        </c:forEach>
                    </select>
                </td>
                    ...
                    <c:forEach items="${colors}" var="color">
                        <input type="checkbox" value="${color.id}"
                                <c:forEach items="${c}" var="c">
                                    <c:if test="${color.id ==c}"> checked="checked"</c:if>
                                </c:forEach>
                ...
                <td width="80%" class="pn-fcontent">
                    <input type="checkbox" name="sizes"
                            <c:forEach items="${s}" var="s">
                                <c:if test="${s =='S'}"> checked="checked"</c:if>
                            </c:forEach>
                           value="S"/>S
                    <input type="checkbox" name="sizes"
                            <c:forEach items="${s}" var="s">
                                <c:if test="${s =='M'}"> checked="checked"</c:if>
                            </c:forEach>
                           value="M"/>M
                    <input type="checkbox" name="sizes"
                            <c:forEach items="${s}" var="s">
                                <c:if test="${s =='L'}"> checked="checked"</c:if>
                            </c:forEach>
                           value="L"/>L
                    <input type="checkbox" name="sizes"
                            <c:forEach items="${s}" var="s">
                                <c:if test="${s =='XL'}"> checked="checked"</c:if>
                            </c:forEach>
                           value="XL"/>XL
                    <input type="checkbox" name="sizes"
                            <c:forEach items="${s}" var="s">
                                <c:if test="${s =='XXL'}"> checked="checked"</c:if>
                            </c:forEach>
                           value="XXL"/>XXL
                </td>
            </tr>
            <tr>
                ...
                <td width="80%" class="pn-fcontent">
                    <input type="checkbox" name="isNew"
                           <c:if test="${product.isNew}">checked="checked"</c:if> value="1"/>新品
                    <input type="checkbox" name="isCommend"
                           <c:if test="${product.isCommend}">checked="checked"</c:if> value="1"/>推荐
                    <input type="checkbox" name="isHot"
                           <c:if test="${product.isHot}">checked="checked"</c:if> value="1"/>热卖
                </td>
            </tr>
            </tbody>
                ...
                <tr>
                    ...
                    <td width="80%" class="pn-fcontent">
                        <c:forEach items="${imgs}" var="img">
                            <img width="100" height="100" name="imgUrl" src="${img}"/>
                        </c:forEach>
